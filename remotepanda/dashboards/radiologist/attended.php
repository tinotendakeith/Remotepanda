...
function rp_done_h($value): string
{
    return htmlspecialchars((string) $value, ENT_QUOTES, 'UTF-8');
}

function rp_done_label($value): string
{
...
$where = "(
    s.status IN ('Finalized','Reported','Returned','Completed')
    OR LOWER(COALESCE(r.status, '')) IN ('reported','returned','finalized')
)";
$where .= " AND {$acl['sql']}";
$types = $acl['types'];
$params = $acl['params'];
...
$sql = "SELECT s.study_id, s.accession_number AS study_accession, s.studyint, s.study AS study_name, s.requested_procedure,
               s.modality AS study_modality, s.status AS study_status, s.scheduled_date, s.assignment_updated_at, s.creation_time,
               r.order_uid, r.clinic_id, r.branch, r.status, r.reported_at, r.returned_at, r.updated_at
        FROM study s
        {$latestOrderJoin}
        WHERE {$where}
        ORDER BY COALESCE(r.reported_at, r.returned_at, s.assignment_updated_at, s.creation_time) DESC
        LIMIT 200";
...
if (empty($cases) && $q === '') {
    $fallbackSql = "SELECT s.study_id, s.accession_number AS study_accession, s.studyint, s.study AS study_name, s.requested_procedure,
               s.modality AS study_modality, s.status AS study_status, s.scheduled_date, s.assignment_updated_at, s.creation_time,
               r.order_uid, r.clinic_id, r.branch, r.status, r.reported_at, r.returned_at, r.updated_at
        FROM study s
        {$latestOrderJoin}
        ...
        ORDER BY COALESCE(r.reported_at, r.returned_at, s.assignment_updated_at, s.creation_time) DESC
        LIMIT 200";
    $fallbackRes = mysqli_query($con, $fallbackSql);
    if ($fallbackRes) {
...
foreach ($cases as $caseRow) {
    $caseStatus = strtolower((string) (($caseRow['status'] ?? '') ?: ($caseRow['study_status'] ?? '')));
...
<html>
<head>
<title>RemotePanda | Finalized Studies</title>
...
.case-stats { display:grid; grid-template-columns:repeat(3, minmax(130px, 1fr)); gap:12px; min-width:470px; }
...
.case-table { width:100%; min-width:1120px; margin:0; border-collapse:collapse; }
...
.case-pill-done { background:#e7f7ed; color:#1f7a3e; }
.case-empty { color:#4c6687; padding:28px !important; text-align:center; }
@media (max-width: 900px) { .case-heading { flex-direction:column; align-items:flex-start; } .case-stats { min-width:0; width:100%; grid-template-columns:1fr; } }
</style>
</head>
  ...
  <div class="case-shell">
    <section class="case-heading">
      <div>
        ...
        <h1>Finalized Studies</h1>
        <p class="case-subtext">Completed cases reported by this radiologist and queued or returned to clinics.</p>
      </div>
      <div class="case-stats">
        <div class="case-stat"><span>Total</span><strong><?php echo number_format(count($cases)); ?></strong></div>
        <div class="case-stat"><span>Reported</span><strong><?php echo number_format($reportedCount); ?></strong></div>
        <div class="case-stat"><span>Returned</span><strong><?php echo number_format($returnedCount); ?></strong></div>
      </div>
    </section>

    <section class="case-card">
      <div class="case-card-header">
        <div><h3>Find Finalized Studies</h3><p class="case-subtext">Search by accession, procedure, modality, or clinic.</p></div>
      </div>
      <div class="case-card-body">
        <form method="get" class="case-filter">
          <div class="case-field"><label>Search</label><input type="text" name="q" class="form-control" value="<?php echo rp_done_h($q); ?>" placeholder="Accession, study, modality, clinic"></div>
          <button class="btn btn-primary" type="submit"><i class="fa fa-search"></i> Search</button>
          <a class="btn btn-default" href="attended.php">Reset</a>
        </form>
      </div>
    </section>
    ...
    <section class="case-card">
      <div class="case-card-header">
        <div><h3>Completed Case Log</h3><p class="case-subtext">Patient contact details are omitted; this is a reporting activity log.</p></div>
      </div>
      <div class="case-card-body">
        <div class="case-table-wrap">
          <table class="table case-table">
            <thead><tr><th>#</th><th>Accession</th><th>Study</th><th>Modality</th><th>Clinic</th><th>Reported</th><th>Status</th><th>Action</th></tr></thead>
            <tbody>
            <?php if (!empty($cases)) { $i = 1; foreach ($cases as $row) {
                $accession = (string) ($row['study_accession'] ?? '');
                ...
                $reportedAt = (string) (($row['reported_at'] ?? '') ?: (($row['returned_at'] ?? '') ?: (($row['assignment_updated_at'] ?? '') ?: ($row['creation_time'] ?? ''))));
            ?>
              <tr>
                <td><?php echo $i; ?></td>
                <td><div class="case-title"><?php echo rp_done_h($accession); ?></div><div class="case-muted"><?php echo rp_done_h($row['studyint'] ?? ''); ?></div></td>
                <td><?php echo rp_done_h($studyName); ?></td>
                <td><span class="case-pill"><?php echo rp_done_h(rp_done_label($modality)); ?></span></td>
                <td><div class="case-title"><?php echo rp_done_h($row['clinic_id'] ?? ''); ?></div><div class="case-muted"><?php echo rp_done_h($row['branch'] ?? ''); ?></div></td>
                <td><?php echo rp_done_h($reportedAt); ?></td>
                <td><span class="case-pill case-pill-done"><?php echo rp_done_h(rp_done_label($status)); ?></span></td>
                <td><a class="btn btn-default" href="view-appointment.php?viewid=<?php echo urlencode($accession); ?>"><i class="fa fa-eye"></i> Review Case</a></td>
              </tr>
            <?php $i++; }} else { ?>
              <tr><td colspan="8" class="case-empty">No finalized studies found.</td></tr>
            <?php } ?>
            </tbody>
          </table>
        </div>
